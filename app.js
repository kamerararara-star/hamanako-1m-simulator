...
async function run(){const n=10000;$('liveSim').disabled=true;$('liveMsg').textContent='実戦MC計算中…';try{if(!liveRaceData)throw new Error('先に公式データを取得してください');const list=liveRaceData.boats||[];const ready=list.length===6&&list.every(b=>b.exhibition_time!=null&&b.exhibition_st!=null&&b.motor_no!=null&&b.motor_2rentai_rate!=null&&b.motor_3rentai_rate!=null);if(!ready)throw new Error('公式6艇データ（展示・ST・モーター）が揃っていません');const res=await fetch(API+'/live/simulate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({race:liveRaceData,simulations:n,seed:Date.now()%2147483647,fronting:fronting(),entry_order:entryOrder})});if(!res.ok)throw new Error(await res.text());const x=await res.json();render(x.result||x);$('simBadge').textContent='10,000 samples';$('liveMsg').textContent='実戦シミュレーション完了';}catch(e){$('liveMsg').textContent='シミュレーション失敗: '+e.message;}}
